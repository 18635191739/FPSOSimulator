...
namespace FPSOSimulator
{
    ...
    {
        //数据库连接字符串（注意：这里的“DBConnectionString”一定要与App.config文件中connectionStrings节点值一致）
        public static readonly string connectionString = ConfigurationManager.ConnectionStrings["MySqlConnection"].ToString();
        
        // 用于缓存参数的HASH表
        private static Hashtable parmCache = Hashtable.Synchronized(new Hashtable());

        private static string name = "MySqlConnection";

        /// <summary>
        /// 建立conn的泛型类方法
        /// </summary>
        /// <param name="name">配置文件的连接字符串名称</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 用指定的数据库连接字符串执行一个命令并返回一个数据集的第一行第一列
        /// </summary>
        /// <remarks>
        ///例如:
